"""
VOLGUARD 3.0 - PRODUCTION HARDENED
Bulletproof option selling system with complete error handling and resilience.
"""
...
class ProductionConfig:
    ...
    # DRY RUN MODE - Set to TRUE for paper trading
...
class TelegramAlerter:
    ...
    def send(self, message: str, level: str = "INFO", retry: int = 3):
        ...
        full_msg = f"{prefix} *VOLGUARD 3.0*\n{message}"
...
class AnalyticsEngine:
    ...
    def run(self, config: Dict):
        try:
            ...
            nifty_response = history_api.get_historical_candle_data(
                instrument_key=ProductionConfig.NIFTY_KEY,
                unit="days", interval=1, to_date=to_date, from_date=from_date
            )
            vix_response = history_api.get_historical_candle_data(
                instrument_key=ProductionConfig.VIX_KEY,
                unit="days", interval=1, to_date=to_date, from_date=from_date
            )
            ...
            live_prices = market_api.get_ltp(
                instrument_key=f"{ProductionConfig.NIFTY_KEY},{ProductionConfig.VIX_KEY}"
            )
    ...
    def _get_expiries(self, options_api: OptionsApi) -> Tuple[Optional[date], Optional[date], Optional[date], int]:
        try:
            ...
            expiry_dates = sorted(list(set([
                datetime.strptime(c.expiry.split('T')[0], "%Y-%m-%d").date()
...
class StrategyFactory:
    ...
    def _calculate_wing_width(self, vol_metrics: VolMetrics, dte: int, spot: float) -> int:
        ...
        return min(max(base_width, ProductionConfig.IRON_FLY_MIN_WING_WIDTH), ProductionConfig.IRON_FLY_MAX_WING_WIDTH)
    ...
    def _find_atm_leg(self, df: pd.DataFrame, atm_strike: float, type_: str) -> Optional[Dict]:
        ...
        spread = (ask - bid) / ltp
        if spread > ProductionConfig.MAX_BID_ASK_SPREAD * 1.5:
    ...
    def generate(self, mandate: TradingMandate, chain: pd.DataFrame, lot_size: int, vol_metrics: VolMetrics, spot: float) -> List[Dict]:
        ...
        if mandate.suggested_structure == "IRON_FLY":
            ...
            max_risk = (max_spread - sum(l['ltp'] for l in legs if l['side'] == 'SELL') + sum(l['ltp'] for l in legs if l['side'] == 'BUY')) * qty
            ...
            legs = [
                {**call_short, 'side': 'SELL', 'role': 'CORE', 'qty': qty, 'structure': 'IRON_CONDOR'},
                {**put_short, 'side': 'SELL', 'role': 'CORE', 'qty': qty, 'structure': 'IRON_CONDOR'},
                {**call_long, 'side': 'BUY', 'role': 'HEDGE', 'qty': qty, 'structure': 'IRON_CONDOR'},
                {**put_long, 'side': 'BUY', 'role': 'HEDGE', 'qty': qty, 'structure': 'IRON_CONDOR'}
            ]
            ...
            legs = [
                {**put_short, 'side': 'SELL', 'role': 'CORE', 'qty': qty, 'structure': 'CREDIT_SPREAD'},
                {**put_long, 'side': 'BUY', 'role': 'HEDGE', 'qty': qty, 'structure': 'CREDIT_SPREAD'}
            ]
...
class ExecutionEngine:
    ...
    def get_order_status(self, order_id: str) -> Optional[Dict]:
        ...
        try:
            ...
            response = order_api.get_order_details(order_id=order_id)
        ...
        for attempt in range(ProductionConfig.MAX_API_RETRIES):
            try:
                ...
                rules = [
                    ...
                    upstox_client.GttRule(strategy="TARGET", trigger_type=target_trigger, trigger_price=float(target_price))
                ]
    ...
    def exit_all_positions(self, tag: Optional[str] = None) -> bool:
        ...
        for attempt in range(ProductionConfig.MAX_API_RETRIES):
            try:
                order_api = OrderApi(self.api_client)
                response = order_api.exit_positions(tag=tag)
...
class StartupReconciliation:
    ...
    def reconcile(self) -> Optional[List[Dict]]:
        ...
        try:
            ...
            portfolio_api = PortfolioApi(self.api_client)
            pos_response = portfolio_api.get_positions()
            ...
            order_api = OrderApi(self.api_client)
            trade_response = order_api.get_trade_history()
...
class SessionManager:
    ...
    def check_market_status(self) -> bool:
        ...
        try:
            ...
            holiday_response = market_api.get_holiday(date=today_str)
