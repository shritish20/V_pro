...
class ProductionConfig:
    ...
    # Iron Fly specific parameters
    IRON_FLY_MIN_WING_WIDTH = 100  # Minimum strike spacing
    IRON_FLY_MAX_WING_WIDTH = 400  # Maximum strike spacing
    IRON_FLY_WING_DELTA_TARGET = 0.10  # Target delta for protective wings
    IRON_FLY_ATM_TOLERANCE = 0.02  # 2% tolerance for ATM selection
    ...
    STOP_LOSS_PCT = 1.0
    MAX_SHORT_DELTA = 0.20  # Reduced for ATM strategies
    EXIT_DTE = 1
...
class TelegramAlerter:
    ...
    def send(self, message: str, level: str = "INFO"):
        ...
        full_msg = f"{prefix} *VOLGUARD v43.0*\n{message}"
        try:
...
class UpstoxAPIClient:
    ...
    def place_order(self, instrument_key: str, qty: int, side: str, order_type: str = "LIMIT", price: float = 0.0) -> Optional[str]:
        try:
            api_instance = upstox_client.OrderApiV3(self.api_client)
            body = upstox_client.PlaceOrderV3Request(quantity=int(qty), product="D", validity="DAY", price=float(price), tag="VG43", instrument_token=instrument_key, order_type=order_type, transaction_type=side, disclosed_quantity=0, trigger_price=0.0, is_amo=False)
...
class RegimeEngine:
    ...
    def generate_mandate(self, score: RegimeScore, vol: VolMetrics, struct: StructMetrics, edge: EdgeMetrics, external: ExternalMetrics, time: TimeMetrics, expiry_type: str, expiry_date: date, dte: int) -> TradingMandate:
        ...
        # Updated strategy selection (NO STRANGLE)
        if score.composite >= 7.5 and dte > 2:
            ...
            rationale.append(f"High Confidence ({score.confidence}): Weighted VRP {w_vrp:.2f} | Far DTE allows wide wings")
            ...
            rationale.append(f"High VRP ({w_vrp:.2f}) + Near DTE → ATM Iron Fly for max theta")
            ...
            rationale.append(f"Moderate Confidence: VRP {w_vrp:.2f} | Balanced risk-reward")
        elif score.composite >= 6.0 and dte <= 1:
            regime_name = "MODERATE_SHORT_GAMMA"; allocation = 35.0; strategy = "MODERATE_SHORT"; suggested = "IRON_FLY"
            rationale.append(f"Moderate VRP near expiry → Iron Fly with tight stops")
        elif score.composite >= 4.0:
            regime_name = "DEFENSIVE"; allocation = 20.0; strategy = "DEFENSIVE"; suggested = "CREDIT_SPREAD"
            rationale.append("Defensive Posture: Defined risk directional play")
        else:
        ...
        if dte <= ProductionConfig.GAMMA_DANGER_DTE and expiry_type == "WEEKLY":
            ...
            if suggested == "IRON_FLY":
                warnings.append("Iron Fly validated for gamma week - tight monitoring enabled")
            allocation *= 0.6
...
# ==========================================
# STRATEGY FACTORY (IRON FLY ENHANCED)
# ==========================================
class StrategyFactory:
    def __init__(self, api: UpstoxAPIClient):
        self.api = api
    
    def _calculate_wing_width(self, vol_metrics: VolMetrics, dte: int, spot: float) -> int:
        """Dynamic wing width based on volatility and time"""
        base_width = ProductionConfig.IRON_FLY_MIN_WING_WIDTH
        
        # Increase wing width with higher volatility
        if vol_metrics.ivp_1yr > 70:
            base_width = int(base_width * 1.5)
        elif vol_metrics.ivp_1yr > 50:
            base_width = int(base_width * 1.2)
        
        # Adjust for DTE (tighter wings near expiry)
        if dte <= 1:
            base_width = max(ProductionConfig.IRON_FLY_MIN_WING_WIDTH, int(base_width * 0.7))
        elif dte <= 3:
            base_width = int(base_width * 0.85)
        
        # Ensure within bounds
        return min(max(base_width, ProductionConfig.IRON_FLY_MIN_WING_WIDTH), 
                   ProductionConfig.IRON_FLY_MAX_WING_WIDTH)
    
    def _find_atm_strike(self, df: pd.DataFrame, spot: float) -> Optional[float]:
        """Find true ATM strike within tolerance"""
        df['atm_diff'] = (df['strike'] - spot).abs()
        df['atm_pct'] = df['atm_diff'] / spot
        
        # Filter strikes within ATM tolerance
        atm_candidates = df[df['atm_pct'] <= ProductionConfig.IRON_FLY_ATM_TOLERANCE]
        
        if atm_candidates.empty:
            logger.warning(f"No ATM strikes within {ProductionConfig.IRON_FLY_ATM_TOLERANCE*100}% tolerance")
            return None
        
        # Find strike with best liquidity
        atm_candidates['liquidity_score'] = atm_candidates['ce_oi'] + atm_candidates['pe_oi']
        best_atm = atm_candidates.nlargest(1, 'liquidity_score').iloc[0]
        
        return best_atm['strike']
    
    def _find_wing_by_width(self, df: pd.DataFrame, atm_strike: float, type_: str, wing_width: int) -> Optional[Dict]:
        """Find wing strike at specific width from ATM"""
        target_strike = atm_strike + wing_width if type_ == 'CE' else atm_strike - wing_width
        
        # Find closest strike to target
        df['wing_diff'] = (df['strike'] - target_strike).abs()
        candidates = df.nsmallest(3, 'wing_diff')
        
        for _, row in candidates.iterrows():
            bid = row[f'{type_.lower()}_bid']
            ask = row[f'{type_.lower()}_ask']
            ltp = row[f'{type_.lower()}_ltp']
            oi = row[f'{type_.lower()}_oi']
            
            # Strict liquidity requirements for wings
            if ltp <= 0 or ask <= 0 or oi < 100:
                continue
            
            spread = (ask - bid) / ltp if ltp > 0 else 1.0
            if spread > ProductionConfig.MAX_BID_ASK_SPREAD:
                logger.warning(f"Skipping {type_} wing {row['strike']}: Spread {spread*100:.1f}%")
                continue
            ...
            return {
                ...
                'strike': row['strike'],
                'ltp': ltp,
                'delta': row[f'{type_.lower()}_delta'],
                'type': type_,
                'oi': oi
            }
    ...
    def _find_atm_leg(self, df: pd.DataFrame, atm_strike: float, type_: str) -> Optional[Dict]:
        """Find ATM leg with liquidity validation"""
        ...
        if ltp <= 0 or ask <= 0:
            logger.error(f"ATM {type_} {atm_strike} has no valid price")
            return None
        
        spread = (ask - bid) / ltp
        if spread > ProductionConfig.MAX_BID_ASK_SPREAD * 1.5:  # Allow slightly wider spread for ATM
            logger.warning(f"ATM {type_} {atm_strike}: Wide spread {spread*100:.1f}%")
    ...
    def generate(self, mandate: TradingMandate, chain: pd.DataFrame, lot_size: int, vol_metrics: VolMetrics, spot: float) -> List[Dict]:
        ...
        if mandate.suggested_structure == "IRON_FLY":
            ...
            # Find ATM strike
            atm_strike = self._find_atm_strike(chain, spot)
            if not atm_strike:
                logger.error("Iron Fly construction failed: No valid ATM strike")
                return []
            
            # Calculate dynamic wing width
            wing_width = self._calculate_wing_width(vol_metrics, mandate.dte, spot)
            logger.info(f"ATM Strike: {atm_strike} | Wing Width: {wing_width}")
            
            # Build Iron Fly: Sell ATM straddle, Buy OTM wings
            atm_call = self._find_atm_leg(chain, atm_strike, 'CE')
            atm_put = self._find_atm_leg(chain, atm_strike, 'PE')
            wing_call = self._find_wing_by_width(chain, atm_strike, 'CE', wing_width)
            wing_put = self._find_wing_by_width(chain, atm_strike, 'PE', wing_width)
            ...
            # Calculate max risk
            call_spread_width = wing_call['strike'] - atm_strike
            put_spread_width = atm_strike - wing_put['strike']
            max_risk = max(call_spread_width, put_spread_width) * qty
            
            logger.info(f"✅ Iron Fly: ATM={atm_strike} | Wings=±{wing_width} | Max Risk=₹{max_risk:,.0f}")
        
        elif mandate.suggested_structure == "IRON_CONDOR":
            logger.info(f"🦅 Constructing Iron Condor | DTE={mandate.dte}")
            
            # Iron Condor: OTM credit spreads on both sides
            call_short = self._find_leg_by_delta(chain, 'CE', 0.20)
            put_short = self._find_leg_by_delta(chain, 'PE', 0.20)
            call_long = self._find_leg_by_delta(chain, 'CE', 0.05)
            put_long = self._find_leg_by_delta(chain, 'PE', 0.05)
            
            if not all([call_short, put_short, call_long, put_long]):
                logger.error("Iron Condor incomplete")
                return []
            
            legs = [
                {**call_short, 'side': 'SELL', 'role': 'CORE', 'qty': qty},
                {**put_short, 'side': 'SELL', 'role': 'CORE', 'qty': qty},
                {**call_long, 'side': 'BUY', 'role': 'HEDGE', 'qty': qty},
                {**put_long, 'side': 'BUY', 'role': 'HEDGE', 'qty': qty}
            ]
        
        elif mandate.suggested_structure == "CREDIT_SPREAD":
            logger.info(f"📊 Constructing Credit Spread (Bearish Bias)")
            
            # Put credit spread
            put_short = self._find_leg_by_delta(chain, 'PE', 0.25)
            put_long = self._find_leg_by_delta(chain, 'PE', 0.10)
            
            if not all([put_short, put_long]):
                logger.error("Credit Spread incomplete")
                return []
        ...
        for leg in legs:
            if leg['ltp'] <= 0:
                logger.error(f"Invalid leg: {leg['strike']} {leg['type']} has no price")
                return []
        
        return legs
    
    def _find_leg_by_delta(self, df: pd.DataFrame, type_: str, target_delta: float) -> Optional[Dict]:
        """Legacy delta-based leg finder for Iron Condor/Credit Spread"""
...
class ExecutionEngine:
    ...
    def _execute_leg_atomic(self, leg):
        ...
        logger.info(f"PLACING {leg['side']} {leg['strike']} {leg['type']} @ {limit_price}")
    ...
    def execute_strategy(self, legs: List[Dict]) -> List[Dict]:
        ...
        # Log strategy type
        structure = executed[0].get('structure', 'UNKNOWN')
        logger.info(f"✅ STRATEGY DEPLOYED: {structure} with {len(executed)} legs")
        telegram.send(f"Position opened: {structure} ({len(executed)} legs)", "TRADE")
        return executed
...
# ==========================================
# RISK MANAGER (ENHANCED FOR IRON FLY)
# ==========================================
class RiskManager:
    def __init__(self, api: UpstoxAPIClient, legs, expiry_date):
        ...
        if structure == 'IRON_FLY':
            ...
            if len(call_strikes) >= 2 and len(put_strikes) >= 2:
                ...
                self.max_spread_loss = (max_spread - self.net_premium) * legs[0]['qty']
                logger.info(f"Iron Fly Max Risk: ₹{self.max_spread_loss:,.0f}")
            else:
        ...
        while self.running:
            try:
                ...
                for leg in self.legs:
                    ...
                    # Track individual leg loss (for ATM legs in Iron Fly)
                    if leg['side'] == 'SELL' and pnl < 0:
                        max_single_leg_loss = min(max_single_leg_loss, pnl)
                
                # Iron Fly specific: Exit if one ATM leg breaches 2x premium
                if self.legs[0].get('structure') == 'IRON_FLY':
                    if abs(max_single_leg_loss) > (self.net_premium * 2.0):
                        self.flatten_all("IRON_FLY_SINGLE_LEG_BREACH")
                        return
                
                # Standard stop loss checks
...
class TradingOrchestrator:
    ...
    def run_analysis(self) -> Optional[Dict]:
        ...
        logger.info(f"Analysis complete - Weekly: {weekly_score.composite:.2f} ({weekly_mandate.suggested_structure}), Monthly: {monthly_score.composite:.2f} ({monthly_mandate.suggested_structure})")
    ...
    def execute_best_mandate(self, analysis: Dict) -> Optional[str]:
        ...
        # Pass vol_metrics and spot to factory
        legs = self.factory.generate(mandate, chain, analysis['lot_size'], vol_metrics, vol_metrics.spot)
        if not legs:
        ...
        trade_id = f"VG43_{int(datetime.now().timestamp())}"
    ...
    def run_auto_mode(self):
        ...
        telegram.send("Auto-trading activated (v43.0 - Iron Fly Enhanced)", "SYSTEM")
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="VOLGUARD v43.0 - Iron Fly Enhanced")
    ...
    try:
        ...
        sys.exit(1)
    db.set_state("system_version", "43.0-IRON_FLY")
    logger.info("Database initialized (WAL Mode enabled)")
    telegram.send("System startup successful (v43.0 - STRANGLE REMOVED | IRON FLY OPTIMIZED)", "SUCCESS")
    ...
    try:
        if args.mode == 'analysis':
            ...
            if result:
                logger.info("=" * 80)
                logger.info(f"Weekly Mandate: {result['weekly_mandate'].regime_name} → {result['weekly_mandate'].suggested_structure}")
                logger.info(f"Monthly Mandate: {result['monthly_mandate'].regime_name} → {result['monthly_mandate'].suggested_structure}")
                logger.info("=" * 80)
        elif args.mode == 'auto':
